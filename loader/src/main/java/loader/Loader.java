...
public abstract class Loader {
    ...
    // CONSTRUCTORS
    public Loader(final String hdfs_input_directory, final String database_name, final SparkSession spark, boolean flagDBExists, boolean flagCreateDB) {
        this.spark = spark;
    ...
    protected void useOutputDatabase() {
        if (flagCreateDB) {
            if (flagDBExists) { // drop DB first
                spark.sql("DROP DATABASE " + database_name + " CASCADE");
                flagDBExists = !flagDBExists;
                logger.info("Dropped database: " + database_name);
            }
