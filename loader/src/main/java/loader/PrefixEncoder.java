...
public class PrefixEncoder extends Loader {
    ...
    // CONSTRUCTORS
    public PrefixEncoder(String hdfs_input_directory, String database_name, SparkSession spark,
    ...
    public PrefixEncoder(String hdfs_input_directory, String database_name, SparkSession spark,
            boolean flagDBExists, boolean flagCreateDB, String table) {
    ...
    public void load() {
        ...
        spark.sql(String.format("DROP TABLE IF EXISTS %1$s.%2$s", this.dbName, this.getFinalTable()));
        ...
        spark.sql(String.format("CREATE TABLE IF NOT EXISTS %1$s.%2$s AS SELECT m1.value as s,t.p, m2.value as o, t.otype FROM %1$s.double_dictionary m1, %1$s.double_dictionary m2, %1$s.triples t WHERE m1.key=t.s AND m2.key=t.o  ", this.dbName, this.getFinalTable()));
